...
- **Sale invoice PDF, black & gold** (2026-09-22): `GET /sales/{id}/receipt.pdf` now renders the CastraNova-branded invoice (logo, gold rule + INVOICE pill, gold Sold To / Invoice No boxes, gold-headed table, Sub Total + gold Total Amount, gold-and-black wave foot from `backend/app/assets/invoice-footer-wave.jpg`). Staff and admin download it via `DownloadInvoiceButton` on the sale-complete card (Sell → Sale) and on SALE rows of the customer page's Transactions. `render_sale_receipt`'s signature is pinned by `tests/services/test_receipt_pdf.py` so no cost field can reach the invoice; `FOOTER_LINES` (bank/KPay) is empty until the owner supplies CastraNova's. Design: `docs/superpowers/specs/2026-09-22-invoice-pdf-design.md`.
