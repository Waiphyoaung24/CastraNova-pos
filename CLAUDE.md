...
- **Alembic head: `m038`** (`b2c3d4e5f6a8`). Run `alembic upgrade head` on any DB still on an older revision. This line goes stale fast — several worktrees add migrations concurrently, so trust `alembic heads` over this doc.
