...
| 6. Ship | `create-pr` (+ `git-pushing`) | After review passes | Clean, scoped PRs — one feature/task group per PR, into `dev`. Release by merging `dev` → `production`. Keep `master` fast-forwarded from `dev`; never commit to it directly. |
...
- **Branch model (3 branches).** `dev` is the development trunk — flow is feature branch → PR → `dev`. `production` is the release/deploy branch (created 2026-06-17 from `dev`); promote by merging `dev` → `production`. `master` is the GitHub **default branch** and exists so the public repository landing page shows the real project; keep it fast-forwarded from `dev` (`git push origin origin/dev:master`). Do not commit to `master` directly — it is a mirror of `dev`, not a place work happens.
  - **`master` stopped being the upstream template base on 2026-08-03**, when it was fast-forwarded from `dev` (`50b89bd` → `c74f18c`, 683 commits). Before that it tracked `full-stack-fastapi-template` untouched, which is what made `copier update` able to pull upstream fixes. **That is no longer possible** — re-pointing at the template now requires a fresh copier base from `50b89bd`, which remains recoverable in history. `copier.yml` and `.copier/` are still in the tree but are inert.
...
- **Sale returns are a shared sale-desk action** (2026-09-21): the Return flow moved off the admin-only Stock adjustment page to its own `/returns` page under Sell, visible to staff and admin. `GET /sales/returnable` and `POST /sales/{id}/returns` accept any logged-in user; staff get `SaleReturnStaffPublic`, which redacts the COGS fields (same pattern as `SaleStaffPublic`). Stock adjustment itself (Found / Lost) stays admin-only.
- **Project pull returns** (2026-09-21, no migration): staff and admin can put stock from a settled pull back (`POST /project-pulls/{id}/returns`), and admin Cancel on a waiting pull now puts its stock back instead of 409ing. Returns append `RETURNED` movements with `project_pull_id` that reverse the pull's own `PROJECT_OUT` ones (units via the new `PROJECT_OUT -> IN_STOCK` edge; parts onto the exact batches). What is returnable is read from the ledgers per (pull, product), so create now allows **one PART line per product**. Project COGS nets returns in the month they come back. The Sell → Return page also lists settled project requests (`GET /project-pulls/returnable`) in its picker, so staff can bring pulled stock back from the same screen. Design: `docs/superpowers/specs/2026-09-21-pull-return-design.md`.
- **Sale invoice PDF, black & gold** (2026-09-22): `GET /sales/{id}/receipt.pdf` now renders the CastraNova-branded invoice (logo, gold rule + INVOICE pill, gold Sold To / Invoice No boxes, gold-headed table, Sub Total + gold Total Amount, gold-and-black wave foot from `backend/app/assets/invoice-footer-wave.jpg`). Staff and admin download it via `DownloadInvoiceButton` on the sale-complete card (Sell → Sale) and on SALE rows of the customer page's Transactions. `render_sale_receipt`'s signature is pinned by `tests/services/test_receipt_pdf.py` so no cost field can reach the invoice; Bank/KPay footer lines are a follow-up once the owner supplies CastraNova's details; dates print in shop time (`SHOP_TZ`). Design: `docs/superpowers/specs/2026-09-22-invoice-pdf-design.md`.
