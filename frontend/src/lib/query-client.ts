...
// Queries: only 401 ends the session. By the time a 401 reaches here, the
// request-boundary interceptor has already attempted a refresh and failed, so
// the session is genuinely dead. 403 = authenticated but under-privileged
// (e.g. staff hitting an admin route); never log those out.
const handleQueryError = (error: Error) => {
  if (error instanceof ApiError && error.status === 401) endSession()
}

// Mutations: 401 ends the session (same dead-session rule as queries); an
// offline-origin 409 on replay is diverted to the admin review queue as
// CONFLICT. A live (online) 409 falls through to the screen's own onError
// toast, unchanged.
