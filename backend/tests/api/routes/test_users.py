...
def test_signup_route_removed(client: TestClient) -> None:
    ...
    # With the POST /users/signup handler gone, the path falls through to the
    # /users/{user_id} family (GET/PATCH/DELETE only), so a POST is rejected
    # with 405 Method Not Allowed. Either way, self-registration is impossible.
    r = client.post(
