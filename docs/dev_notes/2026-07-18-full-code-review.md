...
| ⚪ **Still open, unchanged since morning** | H-1, H-5, H-6, H-8, M-1…M-6, M-8, M-9, M-11…M-15, L-1…L-13 |
...
### H-8 · ➕ NEW · Telegram `connect`/`confirm` are unrate-limited outbound-API amplifiers — 📋 DESIGNED + PLANNED 2026-07-20, implementation DEFERRED
`backend/app/api/routes/notifications.py:49` (`POST /telegram/connect`) and `:66` (`POST /telegram/confirm`) carry **no rate limit**. Only `POST /telegram/test` does (`backend/app/core/limiter.py:22`, `TELEGRAM_TEST_RATE_LIMIT = "10/hour"`).

**Status 2026-07-20:** brainstormed → spec → implementation plan, all committed. **No code written; deliberately deferred.** Pick up from:
- **Spec:** `docs/superpowers/specs/2026-07-20-telegram-connect-hardening-design.md` (`95b29cf`)
- **Plan:** `docs/superpowers/plans/2026-07-20-telegram-connect-hardening.md` (`b644e38`) — 5 TDD tasks, 42 steps, ready to execute

**The investigation reframed the finding.** `get_telegram_updates()` returns the bot's *global* un-acknowledged queue, so the response is byte-identical for every caller — meaning the amplification is better removed by caching the fetch than throttled by a rate limit. The plan therefore has five parts, not two: negative `getUpdates` offset (Part 0), TTL cache (Part 1), per-user rate limits (Part 2), delete-on-mint reaping (Part 3), 429 poll-stop on the client (Part 4).

**Found while investigating — a separate defect, arguably worse than H-8 itself:** `get_telegram_updates` posts `json={}`, so per the Bot API it returns the **oldest** 100 unconfirmed updates. Since the code never confirms, a backlog of >100 pushes a freshly-sent `/start` outside the window and **connect fails silently until it ages out at 24h**. Fixed by Part 0 (`offset: -100`).

**Two assumptions the plan does NOT resolve — settle these first:**
1. *"A negative offset does not confirm updates."* The Bot API docs don't state this either way, and no mock can test it. Plan Task 1 Step 8 is a manual `curl` pre-flight with an explicit revert path — if negative offsets *do* confirm, Part 0 trades a rare ceiling for a common cross-worker race and must be dropped.
2. *slowapi resolves route dependencies before `key_func` runs* (needed for the per-user key). Plan Task 4 Step 6 names `test_one_users_limit_does_not_block_another_user` as the proof, with a concrete fallback (decode the JWT `sub` directly in `key_func`) if it fails.

**Owner decisions recorded during design:** keep the `/confirm` limit and the custom `key_func` (defense-in-depth + reusable infra) despite a marginal cost/benefit read; reaping is delete-on-mint, not a scheduled sweep.

**Also surfaced, not fixed:** slowapi's buckets are **per-process** and `backend/Dockerfile:45` runs `--workers 4`, so *every* existing limit is up to 4× looser than it reads (`TELEGRAM_TEST_RATE_LIMIT = "10/hour"` is really up to 40/hour). Making them exact needs Redis, which the stack doesn't run. Out of scope for H-8; worth its own decision.

- **Impact:** `confirm` calls `notify.get_telegram_updates()` — a real outbound call to Telegram's API — on *every* request, and `TelegramConnectCard.tsx:45-53` polls it every 3s for up to 2 minutes (~40 calls per connect attempt, per user). An authenticated user can drive unbounded traffic to a third-party API on the shop's bot token, risking Telegram-side throttling of the whole bot. `connect` being unlimited also grows `telegramconnectcode` rows with no reaping path.
- **Nuance:** a limit on `confirm` is genuinely harder to size than on `test`, precisely because the happy path polls. Size it against the poll rate (e.g. ≥30/min) rather than borrowing `test`'s number.
- **Also note:** the limiter keys on `get_remote_address` (`limiter.py:11`), which behind Traefik is effectively **shop-global, not per-user** — the same caveat already documented for `REFRESH_RATE_LIMIT` (`:14-18`). So `10/hour` on `test` is 10 for the entire shop. Probably fine for a test button; worth knowing it is a shared bucket, and it compounds M-1.
- **Fix:** add limits to both routes; add a reaper (or `ON CONFLICT` reuse) for expired `telegramconnectcode` rows.
...
- **H-8** — 📋 **designed and planned 2026-07-20; implementation deferred.** See the H-8 entry in §2 for the spec/plan paths, the separate `getUpdates` oldest-window defect found during investigation, and the two assumptions to settle before executing.
